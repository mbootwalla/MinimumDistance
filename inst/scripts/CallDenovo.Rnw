...
subj.ir <- subj.ir[order(start(subj.ir))]
tmp <- unique(c(start(subj.ir), end(subj.ir)))
tmp <- tmp[order(tmp)]
starts <- tmp[seq(1, length(tmp), by=2)]
ends <- tmp[seq(2, length(tmp), by=2)]
disj.ir <- IRanges(starts, ends)
matching.subj <- vector("list", length(disj.ir))
for(i in seq_along(matching.subj)){
	query.ir <- disj.ir[i, ]
	tmp <- findOverlaps(query.ir, subj.ir)
...
2.  For hotspots, make a plot for each trio involved.  Is there a more
efficient data summary

Plot the region for an individual with a denovo event.
