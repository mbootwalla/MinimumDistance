...
outdir <- "/amber1/archive/ctsa/beaty_crlmm/devel"
...
k <- 1
while(any(duplicated(samplesheet$Sample_ID))){
	dup.index <- which(duplicated(samplesheet$Sample_ID))
	samplesheet$Sample_ID[dup.index] <- paste(samplesheet$Sample_ID[dup.index], k, sep="_")
}
arrayInfo <- list(barcode=NULL, position="Human610.Quadv1_B.Sentrix.Position")
@

<<constructCallSet,cache=TRUE>>=
cnSet <- crlmm:::construct.Illumina(sampleSheet=samplesheet,
				    arrayNames=samplesheet$filenames,
				    batch=samplesheet$Sample.Plate,
				    path="",
				    arrayInfoColNames=arrayInfo,
				    cdfName=cdfName,
				    verbose=TRUE,
				    saveDate=TRUE) #, outdir=outdir)
@


<<preprocess,cache=TRUE>>=
mixtureParams <- preprocessInf(cnSet,
			       sampleSheet=samplesheet,
			       arrayNames=samplesheet$filenames,
			       arrayInfoColNames=arrayInfo)
@

<<genotypeRS,cache=TRUE>>=
...
<<>>=
q('no')
cnSet <- genotype.Illumina(batch=as.factor(rep(1, nrow(samplesheet)))[index],
...
##x <- matrix(c(13, 17, 26, 18, 29,
##	      42, 48, 57, 41, 59,
##	      34, 31, 36, 22, 41), nr=3, nc=5, byrow=T)
##rowMedians(x)
##apply(x, 2, median)
##e <- x
##r.j <- numeric(nrow(x))
...
##	t <- t+delta
##
##	cdelta <- apply(e, 2L, median)
##	e <- e-matrix(cdelta, nrow=nrow(e), ncol(e), byrow=T)
##	c.k <- c.k+cdelta
##
##	delta <- median(r.j)
##	r.j <- r.j-delta
...
##apply(e, 2, median)
##
####y_jk=mu+r_j + c_k + e_jk
##mu <- median(x-e)
##y_jk - e_jk -mu = r_j + c_km
##
