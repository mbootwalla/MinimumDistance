...
multiplex <- Beaty:::readMultiplexData(fn)
...
multiplex <- Beaty:::mapTaqmanSampleIds(multiplex)
data(trioSets)
...
##load(md)
data(alternativeTrioRanges)
mdr <- md[sampleNames(md) %in% ss(as.character(multiplex$cidr_name)), ]
mdr$state <- Beaty:::trioStateNames()[mdr$argmax]
df <- RangedDataHMM(ranges=ranges(mdr), values=values(mdr))
df22 <- df[chromosome(df)==22, ]
library(RColorBrewer); palette <- brewer.pal(3, "Set2")
df22 <- Beaty:::todf(df22,col=palette)
fig <- xyplot(x=y~midpoint, data=df22, show.coverage=FALSE, ylim=c(-1,7))

## tick marks at probe locations
