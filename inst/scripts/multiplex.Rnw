...
fn <- fns[grep("HS0", fns)]
##trace(readMultiplexData, browser)
...
multiplex$start <- fd$st[match(multiplex$probeid, fd$probeids)]
multiplex$end <- fd$en[match(multiplex$probeid, fd$probeids)]
trace(mapTaqmanSampleIds, browser)
multiplex$gwas.id <- mapTaqmanSampleIds(multiplex)
multiplex <- updateWithGwasFindings(multiplex, fd)
