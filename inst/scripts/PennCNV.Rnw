...
##tmp <- penn.joint$id %in% sampleNames(minDistanceSet)
penn.joint <- penn.joint[penn.joint$id %in% sampleNames(minDistanceSet), ]
penn.denovo <- penn.joint[penn.joint$triostate==332 | penn.joint$triostate==331, ]
@

Two questions.

1. What is the overlap with our list?

2. Are there PennCNV hits that we're missing?

<<>>=
penn.denovo <- penn.denovo[penn.denovo$nmarkers >= 5, ]
