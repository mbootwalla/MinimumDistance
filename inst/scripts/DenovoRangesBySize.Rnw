...
trace(collectAllRangesOfSize, browser)
deletionRanges <- collectAllRangesOfSize(SIZE=5,
					 bsSet=bsSet,
					 minDistanceSet=minDistanceSet,
					 MIN=1, MAX=4,
					 lambda=0.1)
deletionRanges <- deletionRanges[!noCentromere.overlap, ]
##homozygous (?) sum(deletionRanges$pHet > 0.05 & deletionRanges$seg.mean > 0.5)
deletionRanges <- deletionRanges[deletionRanges$pHet < 0.05 | deletionRanges$seg.mean > 0.5, ]
FRAME.SHIFT <- 200e3
