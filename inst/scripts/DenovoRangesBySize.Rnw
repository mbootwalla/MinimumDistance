% Among the top hits, are there any overlaps with other ranges?
%
% Among the Jugessur ranges, are there any other overlaps?
%
% We're missing a few obvious hits because of the threshold.
%
% Should we try penncnv's tool for calculing p-values of the denovo regions
%
% Adding functional annotation to plots
%
<<>>=
##refGene <- getRefGene(filename="~/Data/Downloads/hg18_refGene.txt")
@
...
featureNames(minDistanceSet) <- rownames(fData(minDistanceSet))
