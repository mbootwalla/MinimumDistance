...
deletion.ranges <- collectAllRangesOfSize(SIZE=5,
					 bsSet=bsSet,
					 minDistanceSet=minDistanceSet,
					 MIN=1, MAX=4,
					 lambda=0.1)
deletion.ranges <- deletion.ranges[deletion.ranges$noCentromere.overlap, ]
##homozygous (?) sum(deletion.ranges$pHet > 0.05 & deletion.ranges$seg.mean > 0.5)
deletion.ranges <- deletion.ranges[deletion.ranges$pHet < 0.05 | ( deletion.ranges$seg.mean > 1 & deletion.ranges$num.mark > 15), ]
index <- which(deletion.ranges$pHet > 0.05 & deletion.ranges$seg.mean > 1)
...
save(deletion.ranges, file="~/projects2/BeatyExperimentData/data/deletion.ranges.rda")
@
...
pdf("chromosomeRanges_v2%02d.pdf", width=8, height=8, onefile=FALSE)
par(las=1, mar=c(0.5, 6, 0.5 , 0.5), oma=c(2, 0.5, 4, 1))
layout(matrix(1:2, 2, 1), heights=c(1, 0.2))
